...
export default function TradingForm({ selectedCoin, selectedCoinData, onTradeSuccess }: TradingFormProps) {
  const { user } = useAuth();
  ...
  const fetchPortfolio = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/portfolio/${user?.id}`, {
        headers: {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/orders/${portfolio.id}`, {
        headers: {
          'x-user-id': user?.id?.toString() || ''
        }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!portfolio || !user?.id || !quantity) return;

    setIsLoading(true);
    try {
      // Create order
      const orderData = {
        portfolio_id: parseInt(portfolio.id),
      ...
      if (type === 'market') {
        ...
        const executeResponse = await fetch(`${API_BASE_URL}/orders/${order.id}/execute`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-user-id': user.id.toString()
