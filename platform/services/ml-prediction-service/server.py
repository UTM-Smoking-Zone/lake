...
class MLFeature(BaseModel):
    ...
    @validator('high', 'sma_20', 'sma_50', 'sma_200', 'bb_middle', 'bb_upper',
               'volatility_10d', 'volatility_14d', 'macd_12_26', 'macd_5_35',
               'macd_signal_5_35')
...
async def health_check():
    ...
    if model_load_error:
        logger.warning("Health check: Model loading failed")
        return {
            "status": "degraded",
...
async def predict(request: PredictionRequest):
    ...
    if model is None or scaler is None:
        ...
        raise HTTPException(
            status_code=503,
            detail="Model or scaler not loaded. Service degraded."
        )
